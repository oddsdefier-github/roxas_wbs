            ...
            `<li title="Address">
                    <div class="flex items-center p-2 rounded hover:bg-gray-100">
                        <label for="${address.brgy}" class="w-full text-sm font-medium text-gray-600 rounded dark:text-gray-300">
                            <input id="${address.brgy}" type="radio" value="${address.brgy}" data-column="brgy" name="brgy" class="mr-1 w-4 h-4 peer text-yellow-400 bg-gray-100 border-gray-300 focus:ring-transparent">
...
$(document).ready(function () {
    ...
    $('input[type="radio"]').each(function () {
        $(this).on('change', function () {
            ...
            const clientTable = new DataTableWithPagination("client_data", '#displayClientForBilling', { column: column, value: value });
        });
    })
